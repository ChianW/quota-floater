...
async function detectProcessInfosWin32(deps = {}) {
  ...
  // Filter in WMI. Enumerating every Win32_Process then Where-Object routinely
  // exceeds the 8–10s probe deadline on Windows, which Token Monitor treats as
  // "Antigravity not logged in" even when the language server is running.
  const script = `Get-CimInstance Win32_Process -Filter "Name='language_server.exe'" | ForEach-Object { "$($_.ProcessId) $($_.CommandLine)" }`;
  ...
  for (const candidate of candidates) {
    try {
      ...
      if (windows.some((window) => window.remainingFraction !== null)) {
        throwIfAborted(signal);
        return { snapshot: { accountPlan: null, accountEmail: null, windows }, lastError };
      }
